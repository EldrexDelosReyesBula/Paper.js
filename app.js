    ...
    const Hero = () => {
        return paper.div(".hero-wrapper.container",
            ...
            paper.flex.center(
                ...
                paper.a({ href: 'docs.html#/docs.html', class: 'toolbar-btn', style: { padding: '12px 24px', borderRadius: 'var(--radius-md)', textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: '8px' } },
...
paper.toast("This is a real Windows system notification! 💻", "info", 3000, true);`,
        ...
        'oop_components': {
            ...
            icon: 'box',
            blueprint: `class UserCard extends paper.component {
    render() {
        return paper.div(".card", 
            paper.h3("ES6 Class Binding"),
            paper.p("Rendered from an OOP instance!")
        );
    }
}

paper.mount('#app', new UserCard().render());`,
            creator: () => {
                class DemoCard extends paper.component {
                    render() {
                        return paper.div({ style: { padding: '20px', background: 'rgba(255,255,255,0.05)', borderRadius: '12px', border: '1px solid var(--border-glow)' } },
                            paper.h3('OOP Class Component', { style: { marginTop: 0 } }),
                            paper.p('This UI was generated by extending the new `paper.component` base class.'),
                            paper.button('Encapsulated Click', { 
                ...
                return paper.div(
                    paper.h2('Object-Oriented UI', { animate: 'fade-in' }),
                    new DemoCard().render()
                );
            }
        },
        'orm_database': {
            ...
            icon: 'database',
            blueprint: `class User extends paper.model {}
...
User.watch(users => console.log("DB Updated:", users));`,
            creator: () => {
                ...
                paper.db('demo_users').state.value = [];
                
                class DemoUser extends paper.model {}
                // overriding name so it maps to 'demo_users'
                Object.defineProperty(DemoUser, 'name', { value: 'demo_user' });

                return paper.div(
                    paper.h2('Paper ORM + Unified DB', { animate: 'fade-in' }),
                    paper.p('paper.db and paper.model provide instant data persistence.'),
                    
                    // Input to create user
                    paper.div({ style: { display: 'flex', gap: '10px', marginBottom: '20px' } },
                        paper.input({ id: 'demo-db-input', placeholder: 'Enter username...', class: 'paper-input', style: { padding: '8px', borderRadius: '6px', border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(0,0,0,0.2)', color: 'white' } }),
                        paper.button('Insert Record', {
                            class: 'btn-primary',
                            onclick: () => {
                    ...
                    paper.div(".db-output", { style: { background: '#0a0f1c', padding: '15px', borderRadius: '8px', border: '1px solid var(--border-glow)' } },
                        paper.h4('Reactive Storage Output:'),
                        paper.div(() => {
                            let records = DemoUser.all();
                            if (records.length === 0) return paper.p('Database is empty.', { style: { color: 'var(--text-muted)' } });
                            return paper.ul(
                                ...records.map(r => paper.li(`ID: ${r.id} | Name: ${r.name} | Time: ${r.timestamp}`))
                            );
                        })
                    )
                );
        ...
        return paper.footer(".main-footer",
            paper.div(".container",
                paper.div(".social-links",
                    ...
                    paper.a({ href: 'docs.html#/docs.html', style: { display: 'inline-flex', alignItems: 'center', gap: '6px', textDecoration: 'none' } },
